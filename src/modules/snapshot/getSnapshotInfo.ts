...
export class GetSnapshotInfoConsumer implements IConsumer {
  ...
  public async exec(msg: string, channel: Channel, brokerMsg: Message) {
    try {
      ...
      const minerInfo = await this.lotus.walletProvider.minerInfo(miner, message.tipset);
      const minerPower = await this.lotus.walletProvider.minerPower(miner, message.tipset);
      const minerState = await this.lotus.walletProvider.readState(miner, message.tipset);
      ...
      if (minerInfo.Owner !== minerInfo.Worker) relatedAddresses.push(minerInfo.Worker);
