...
export class PollsServiceCrons {
  ...
  async getPollsFromIssues() {
    ...
    getIssuesReq = this.httpService.get(
      ...
      async (response) => {
        ...
        for (let i = 0; i < issues.length; i++) {
          try {
            ...
            let poll = await this.pollsRepository.findOne({
              fileName: issue.name,
            });
            if (!poll) {
              const issueBody = await this.httpService
                .get(issue.download_url, {
                  auth: {
                    username: this.config.values.github.user,
                    password: this.config.values.github.token,
                  },
              ...
              const issueBody = await this.httpService
                ...
                .toPromise();
              const { description, author, discussion } = parseIssue(
                issueBody.data,
                this.config.values.issueParser,
              );
  ...
  async updatePollStatus() {
    ...
    polls = await getRepository(Poll)
      .createQueryBuilder()
      .where(
        'status = :status and start <= NOW() and Poll.snapshotCreated = :snapshotCreated',
        {
          status: Status.BUILDING_SNAPSHOT,
