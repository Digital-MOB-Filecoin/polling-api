...
import { LotusClient, LotusWalletProvider, WsJsonRpcConnector } from 'filecoin.js'
...
export class PollsServiceEvents {
    ...
    async handleGetPollSnapshot(poll: Poll) {
        ...
        const snapshotTipset = await wsWalletProvider.getTipSetByHeight(poll.snapshotHeight);
        ...
        for (let i = 0; i < miners.length; i++) {
            await this.rabbitMQService.publish('poll', 'getSnapshotInfo', JSON.stringify({
