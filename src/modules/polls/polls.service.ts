...
export class PollsService {
    ...
    async previewIssue(issueId: string) {
        try {
            const getIssuesReq = await this.httpService.get(`https://api.github.com/repos/filecoin-project/community/contents/polls`, {
                auth: {
            ...
            if (!issues || !Array.isArray(issues)) return { message: "Malformed issue" };

            for (let i = 0; i < issues.length; i++) {
                try {
                    if (issues[i].name == issueId) {
                        issueBody = await this.httpService.get(issues[i].download_url).toPromise();
                    };
                }
                catch (e) {
            ...
            if (e.response && e.response.status === 404) return { message: "Issue not found" };
            return { message: "Malformed issue" };
        }
    }

    async getPoll(id: number) {
        const poll = await this.pollsRepository.findOne({ where: { id: id }, relations: ["options", "constituentGroups"] })
        if (!poll) {
        ...
        const allConstituentGroups = await this.constituentGroupRepository.find({ order: { position: 'ASC' } });
        ...
        voteResultsDb.forEach((res: VoteResult) => {
            if (!voteResults[res.constituentGroupId]) voteResults[res.constituentGroupId] = {};
    ...
    async getPollsListActive() {
        const polls = await this.pollsRepository.find({ where: { end: MoreThanOrEqual('NOW()') }, order: { id: "DESC" } })
        return polls;
    }
    ...
    async getConstituentGroupVoteList(id: number, constituentGroupId: number) {
        ...
        let votes = getRepository(Vote)
            .createQueryBuilder("vote");

        if (constituentGroup.identifier === 'tokenholders') {
            votes = votes.where("vote.pollId = :id and (vote.balance > 0 or vote.lockedBalance > 0) and vote.constituentGroupId = 0", {
                id: id,
            });
        }
        if (constituentGroup.identifier === 'storageminers') {
            votes = votes.where("vote.pollId = :id and vote.power > 0 and vote.constituentGroupId = 0", {
                id: id,
            });
        }
        if (constituentGroup.groupType !== GroupType.DEFAULT) {
            if (constituentGroup.voteType === VoteType.SIMPLE) {
                votes = votes.where("vote.pollId = :id and vote.constituentGroupId = :constituentGroupId", {
                    id: id,
                    constituentGroupId: constituentGroup.id,
                });
            }
            if (constituentGroup.voteType === VoteType.WEIGHTED_BY_BALANCE) {
                votes = votes.where("vote.pollId = :id and vote.constituentGroupId = :constituentGroupId and (vote.balance > 0 or vote.lockedBalance > 0)", {
                    id: id,
                    constituentGroupId: constituentGroup.id,
                });
            }
            if (constituentGroup.voteType === VoteType.WEIGHTED_BY_POWER) {
                votes = votes.where("vote.pollId = :id and vote.constituentGroupId = :constituentGroupId and vote.power > 0", {
                    id: id,
                    constituentGroupId: constituentGroup.id,
                });
            }
    ...
    async voteInPoll(id: number, params: VoteParamsDto) {
        ...
        let vote = await this.voteRepository.findOne({ pollId: id, address: params.address });
        if (vote) {
            throw new HttpException({
                status: HttpStatus.BAD_REQUEST,
        ...
        try {
            ...
            const check = await this.lotus.walletProvider.verify(params.address, Buffer.from(message), sig);
            if (!check) {
        ...
        const voter = await this.voterRepository.findOne({ address: params.address });
        let voterGroupId = 0;
        if (voter) {
        ...
        try {
            accountState = await this.lotus.walletProvider.getActor(params.address, JSON.parse(poll.snapshotTipsetKey));
            ...
            throw new HttpException({
                status: HttpStatus.BAD_REQUEST,
                error: 'Error retrieving address for voting. This address is not on chain at the specified snapshot height.',
            }, HttpStatus.BAD_REQUEST);
        }
        ...
        try {
            ...
            throw new HttpException({
                status: HttpStatus.BAD_REQUEST,
                error: 'Error retrieving chain actor. This f0 address cannot be retrieved.',
            }, HttpStatus.BAD_REQUEST);
        }

        const minersInfo: MinerInfo[] = await this.snapshotService.getMinersInfo(addressId, poll.snapshotHeight);

        await this.voteRepository.save(vote);

        await this.rabbitMQService.publish('poll', 'storeVote', JSON.stringify({
            voteId: vote.id,
            pollId: poll.id,
        ...
        for (let i = 0; i < minersInfo.length; i++) {
            ...
            voteAggregatedLockedBalance = voteAggregatedLockedBalance.add(new BN(minerInfo.lockedBalance));

            if (poll.countInMultipleGroups ||
                ...
                minerVote.constituentGroupId =
                    vote.constituentGroupId === 0 || voteConstituentGroup.voteType === VoteType.WEIGHTED_BY_POWER || voteConstituentGroup.voteType === VoteType.WEIGHTED_BY_BALANCE ?
                        vote.constituentGroupId : 0;
                minerVote.balance = "0";
        ...
        if (voterGroupId !== 0 && poll.countInMultipleGroups) retrieveConstituentGroupCriteria = { where: [{ id: voterGroupId }, { groupType: GroupType.DEFAULT }] };
        if (voterGroupId === 0) retrieveConstituentGroupCriteria = { groupType: GroupType.DEFAULT };

        const constituentGroups = await this.constituentGroupRepository.find(retrieveConstituentGroupCriteria);
        ...
        for (let i = 0; i < constituentGroups.length; i++) {
            ...
            if (constituentGroup.groupType === GroupType.LIST) {
                votes = await votesQuery.where("vote.pollId = :id and vote.constituentGroupId = :constituentGroupId and vote.optionId = :optionId", {
                    id: id,
                    constituentGroupId: constituentGroup.id,
                    optionId: option.id
                })
                ...
                votes = await votesQuery.where("vote.pollId = :id and vote.optionId = :optionId", {
                    id: id,
                    optionId: option.id
                })
                    .getRawOne();
            ...
            if (constituentGroup.voteType === VoteType.WEIGHTED_BY_BALANCE) {
                ...
                valueAux = !votes['totalLockedBalance'] ? '0' : votes['totalLockedBalance'];
            }
            ...
            await getRepository(VoteResult)
                ...
                .update(VoteResult)
                .set({ value, valueAux })
                .where("pollId = :pollId and constituentGroupId = :constituentGroupId and optionId = :optionId", {
                    pollId: id,
                    constituentGroupId: constituentGroup.id,
                    optionId: option.id
                })
            ...
            if (constituentGroup.groupType === GroupType.DEFAULT) {
                poll.constituentGroups.forEach(pollConstituentGroup => {
                    console.log(pollConstituentGroup.identifier, constituentGroup.voteType)

                    if (pollConstituentGroup.identifier == 'tokenholders' && constituentGroup.voteType === VoteType.WEIGHTED_BY_BALANCE) {
                    ...
                    if (pollConstituentGroup.identifier == 'storageminers' && constituentGroup.voteType === VoteType.WEIGHTED_BY_POWER) {
